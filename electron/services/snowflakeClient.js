...
exports.upsertPatientSnapshot = upsertPatientSnapshot;
exports.callNurseQuery = callNurseQuery;
exports.getCohortSummary = getCohortSummary;
exports.getLastSyncTime = getLastSyncTime;
exports.isPatientSyncedRecent = isPatientSyncedRecent;
exports.getCensusFromSnowflake = getCensusFromSnowflake;
exports.isSnowflakeAvailable = isSnowflakeAvailable;
...
async function getConnection() {
    ...
    if (!config.account || !config.username) {
        throw new Error("Snowflake not configured. Set SNOWFLAKE_ACCOUNT and SNOWFLAKE_USER in .env");
    }
 ...
 */
async function upsertPatientSnapshot(snapshot) {
    let rowsWritten = 0;
    ...
    if (snapshot.rawResources.length > 0) {
        ...
        for (let i = 0; i < snapshot.rawResources.length; i += BATCH_SIZE) {
            ...
            await executeSql(`INSERT INTO fhir_raw (raw_id, patient_id, resource_type, raw_json, ingested_at)
         ${sqlParts.join(" UNION ALL ")}`, binds);
        }
    }
    // 8. Write snapshot record
    const snapshotId = `snap-${Date.now()}`;
    await executeSql(`INSERT INTO patient_snapshots (snapshot_id, patient_id, snapshot_at, lookback_hours, completeness_flags, resource_counts)
    ...
    console.log(`[Snowflake] Upserted ${rowsWritten} rows for patient ${snapshot.patient?.patient_id}`);
    return { rowsWritten, snapshotId };
}
async function upsertPatient(p) {
    await executeSql(`MERGE INTO patients AS t
...
async function upsertAllergiesBatch(allergies) {
    const BATCH_SIZE = 40;
...
async function upsertMedicationsBatch(meds) {
    const BATCH_SIZE = 40;
...
async function upsertLabsBatch(labs) {
    const BATCH_SIZE = 40;
...
async function upsertVitalsBatch(vitals) {
    const BATCH_SIZE = 40;
 ...
 */
async function callNurseQuery(patientId, question, encounterId) {
 ...
 */
async function getLastSyncTime(patientId) {
    const rows = await executeSql(`
 ...
 */
async function isPatientSyncedRecent(patientId, minutesThreshold = 10) {
    const rows = await executeSql(`
 ...
 */
async function getCensusFromSnowflake() {
    const sql = `
    SELECT
 ...
 */
async function isSnowflakeAvailable() {
    try {
        const config = getConfig();
        if (!config.account || !config.username)
            return false;
...
// ── Helpers ──
function buildCompletenessFlags(snapshot) {
    const flags = {};
    if (snapshot.labs.length === 0) {
    ...
    const missing = criticalLabs.filter((l) => !Array.from(labNames).some((name) => name.includes(l)));
    if (missing.length > 0) {
        flags.missing_critical_labs = missing;
    }
    return flags;
}
function parseCitations(searchResults) {
    if (!searchResults)
        return [];
    try {
...
function parseFlags(completenessFlags) {
    if (!completenessFlags)
        return [];
    const flags = [];
    const parsed = typeof completenessFlags === "string"
        ? JSON.parse(completenessFlags)
        : completenessFlags;
    if (parsed.missing_labs)
        flags.push("No recent labs available");
    if (parsed.stale_vitals)
        flags.push("Vitals data may be stale");
    if (parsed.no_medications)
        flags.push("No medications documented");
    if (parsed.missing_critical_labs) {
        flags.push(`Missing critical labs: ${parsed.missing_critical_labs.join(", ")}`);
    }
    return flags;
}
