...
exports.transformBundle = transformBundle;
 ...
 */
function transformBundle(patientId, bundle, lookbackHours = 72) {
    ...
    const patient = transformPatient(patientId, byType.get("Patient")?.[0] ?? null, byType.get("Condition") ?? []);
    const allergies = (byType.get("AllergyIntolerance") ?? []).map((r) => transformAllergy(patientId, r));
    const medications = (byType.get("MedicationRequest") ?? []).map((r) => transformMedication(patientId, r));
    ...
    for (const obs of observations) {
        ...
        const isVitalCategory = obs.category?.some((cat) => cat.coding?.some((c) => c.code === "vital-signs"));
    ...
    const notes = (byType.get("DocumentReference") ?? []).map((r) => transformNote(patientId, r));
...
function transformPatient(patientId, resource, conditions) {
    if (!resource)
        return null;
    ...
    const diagnosis = primaryCondition?.code?.coding?.[0]?.display ??
        primaryCondition?.code?.text ??
        "Unknown";
    // Summary from all active conditions
    const conditionNames = conditions
        .slice(0, 5)
        .map((c) => c.code?.coding?.[0]?.display ?? c.code?.text ?? "Unknown condition");
    const summary = conditionNames.length > 0
    ...
    const riskScore = Math.min(0.99, 0.3 + conditions.length * 0.1 + Math.random() * 0.15);
    return {
...
function transformAllergy(patientId, resource) {
    const allergen = resource.code?.coding?.[0]?.display ?? resource.code?.text ?? "Unknown";
    const reaction = resource.reaction?.[0]?.manifestation?.[0]?.coding?.[0]?.display ?? "";
    const severity = resource.reaction?.[0]?.severity ?? resource.criticality ?? "unknown";
    return {
...
function transformMedication(patientId, resource) {
...
function transformLab(patientId, resource) {
    const labName = resource.code?.coding?.[0]?.display ?? resource.code?.text ?? "Unknown lab";
    ...
    else if (interp === "A" || interp === "AA" || String(interp).includes("Abnormal"))
        flag = "high";
    return {
        lab_id: resource.id,
        patient_id: patientId,
 ...
 */
function groupVitals(patientId, observations) {
    ...
    for (const obs of observations) {
        ...
        codings.forEach(c => {
            const code = c.code;
            if (code === "8867-4")
                existing.hr = value ?? null;
            else if (code === "8480-6")
                existing.bp_sys = value ?? null;
            else if (code === "8462-4")
                existing.bp_dia = value ?? null;
            else if (code === "9279-1")
                existing.rr = value ?? null;
            else if (code === "8310-5")
                existing.temp = value ?? null;
            else if (code === "2708-6")
                existing.spo2 = value ?? null;
        });
        ...
        if (hasBPPanel) {
            ...
            for (const comp of components) {
                ...
                if (compCodings.some(cc => cc.code === "8480-6"))
                    existing.bp_sys = compValue;
                else if (compCodings.some(cc => cc.code === "8462-4"))
                    existing.bp_dia = compValue;
            }
        }
        byTimestamp.set(roundedDt, existing);
    }
    // Final pass: filter out any rows that somehow ended up with all primary vitals null
    return Array.from(byTimestamp.values()).filter(v => v.hr !== null || v.bp_sys !== null || v.bp_dia !== null ||
        v.rr !== null || v.temp !== null || v.spo2 !== null);
}
function transformNote(patientId, resource) {
    ...
    if (content?.attachment?.data) {
        // Base64-encoded text
        try {
            noteText = Buffer.from(content.attachment.data, "base64").toString("utf-8");
        }
        catch {
    ...
    const author = resource.author?.[0]?.display ?? resource.author?.[0]?.reference ?? "Unknown";
    return {
        note_id: resource.id,
        patient_id: patientId,
