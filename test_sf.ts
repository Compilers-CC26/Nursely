import { executeSql } from "./electron/services/snowflakeClient";
...
// We just copy the internals of executeSql here to avoid module issues testing outside electron
...
conn.connect((err, conn) => {
  ...
  conn.execute({
    ...
    complete: (err, stmt, rows) => {
      ...
      else console.log("Got rows:", rows.length);
      process.exit(0);
    }
  });
});
